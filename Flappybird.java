...
public class Flappybird extends Actor {
    ...
    for (Helicopter helicopter : getWorld().getObjects(Helicopter.class)) {
        if (Math.abs(helicopter.getX() - getX()) < 60 && Math.abs(helicopter.getY() - getY()) < 37) {
            Greenfoot.playSound("peng.mp3");
            isalive = false;
            isacross = true;
        }
